package xhe;

import java.util.Arrays;
import java.util.Scanner;

public class Main {

	public static void main(String[] args) {
		
		Scanner sc = new Scanner(System.in);
		
		Cat catOne = new Cat("Vaska", 3);
		Cat catTwo = new Cat("Umka", 7);
		Cat catThree = new Cat("Kuzia", 5);
		Cat catFour = new Cat("Dimok", 1);
		Cat catFive = new Cat("Rigik", 2);
		
		Cat[] catArray = new Cat[] {catOne, catTwo, catThree, catFour, catFive};
		for (Cat cat : catArray) {
			System.out.println(cat);
		}
		//sorting by age
		Arrays.sort(catArray,(a,b) -> a.getAge() - b.getAge());
		
		System.out.println();
		for (Cat cat : catArray) {
			System.out.println(cat);
		}
		//	alphabetical classification/sorting
		Arrays.sort(catArray,(a,b) -> a.getName().compareTo(b.getName()) );
		
		System.out.println();
		for (Cat cat : catArray) {
			System.out.println(cat);
		}
		
		int parametr = 0;
		System.out.println("Input parametr : ");
		//sorting by parametr
		parametr = sc.nextInt();
		
		if(parametr==1) {//sorting in ascending order
			Arrays.sort(catArray,(a,b) -> a.getAge() - b.getAge());
		}
		if(parametr==2) {//	Sorting in decreasing order
			Arrays.sort(catArray,(a,b) -> -1*(a.getAge() - b.getAge()));
		}
		if (parametr==3) {//alphabetical classification
			Arrays.sort(catArray,(a,b) -> a.getName().compareTo(b.getName()) );
		}
		
		System.out.println();
		for (Cat cat : catArray) {
			System.out.println(cat);
		}
	}

}
